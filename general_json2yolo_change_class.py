...
def convert_coco_json(json_dir='.\coco', use_segments=True, cls91to80=False):
    save_dir = make_dirs()  # output directory
    coco80 = coco91_to_coco80_class()

    # Import json
    for json_file in sorted(Path(json_dir).resolve().glob('*.json')):
        fn = Path(save_dir) / 'labels' / json_file.stem.replace('labels', 'a')  # folder name
        fn.mkdir()
        ...
        for img_id, anns in tqdm(imgToAnns.items(), desc=f'Annotations {json_file}'):
            ...
            for ann in anns:
                flag=0
                if ann['category_id'] not in filter_class:
                    flag=1
                    continue
                if ann['iscrowd']:
                    continue
